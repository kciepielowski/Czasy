...
Przyklczasy::Przyklczasy(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::PrzyklczasyClass)
{
    ui->setupUi(this);
}

Przyklczasy::~Przyklczasy()
{
    delete ui;
}

void Przyklczasy::on_rPast_clicked()
{
      Czas1=0;
  //Visible - setCheckable(bool);
  //Enabled - setEnabled(bool);
  //Checked - setChecked(bool);
  PerfectC->setText("Perfect Continuous");
  PerfectP->setEnabled(false);
  if(Czas2==4||Czas2==5)
  {
   Czas2=3;
      PerfectC->setChecked(true);
      PerfectP->setChecked(false);
  }
}

void Przyklczasy::on_rPresent_clicked()
{
  Czas1=1;
  PerfectC->setText("Perfect Continuous");
  PerfectP->setEnabled(false);
...
void Przyklczasy::on_rFuture_clicked()
{
      Czas1=2;
  PerfectC->setText("In The Past");
  PerfectP->setEnabled(true);
  if(Czas2==3)
  {
   Czas2=4;
  }
...
void Przyklczasy::on_rSimple_clicked()
{
      Czas2=0;
}
void Przyklczasy::on_rContinuous_clicked()
{
    Czas2=1;
}

void Przyklczasy::on_rPerfect_clicked()
{
    Czas2=2;
}

void Przyklczasy::on_rPerfectC_clicked()
{
    Czas1==2?Czas2=4:Czas2=3;
}

void Przyklczasy::on_rPerfectP_clicked()
{
    Czas2=5;
}

void Przyklczasy::on_pOK_clicked()
{
 ...
 if(!slowa[0].isEmpty()&&!slowa[1].isEmpty())
{
 unsigned short a;
 a=slowa[0].length()-1;
 for(int i=2;i<=a;i++)
 {
  if(slowa[0][i]<char(91)&&slowa[0][i]>char(64))slowa[0][i].toLower();
 }
 a=slowa[1].length()-1;
 if(slowa[1]!="I")
 {
 for(int i=1;i<=a;i++)
 {
  if(slowa[1][i]<char(91)&&slowa[1][i]>char(64))slowa[1][i].toLower();
 }
 }
 for(int j=2;;j++)
 {
 ...
 for(int i=1;i<=a;i++)
 {
  if(slowa[j][i]<char(91)&&slowa[j][i]>char(64))slowa[j][i].toLower();
 }
 }else break;
 }
 ...
 if(pod==0)
 {
  ...
  || slowa[0]=="she" || slowa[0]=="It" || slowa[0]=="it")
  {
   if(slowa[cz]!="is")
   {
    ...
    {
    slowa[cz]=slowa[cz].mid(0, a-1);
    --a;
    bool znaleziono=false;
     QString czasownik[2026]={"abase", "abate", "abbreviate", "abdicate",
...
"crease", "create", "cremate", "crenelate", "crepitate", "criminalise",
 ...
 "wrestle", "wriggle", "wrinkle", "write"};
     ...
     if(!znaleziono){slowa[cz]=slowa[cz].mid(0, a-1); --a;}
    }
   }
  }
 }
 ...
 for(int i=1; i<a; i++)
 {
  ...
  if(slowa[cz][a]=='e')
  {
   if(slowa[cz][a-1]=='i'){slowa[cz]=slowa[cz].mid(0, a-1); slowa[cz]+='y';}
   if(slowa[cz]!="age" && slowa[cz]!="dye" && slowa[cz]!="agree"
   && slowa[cz]!="see" && slowa[cz]!="be")slowa[cz]=slowa[cz].mid(0, a);
  }
  slowa[cz]+="ing";
 }
 ...
 a=wynik.length();
 wynik=wynik.mid(0, a-1);
 if(!pytanie){wynik+='.';}else wynik+='?';

 ui->lNoweZdanie->setText(wynik);
}
}
